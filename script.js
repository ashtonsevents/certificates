...
const pharmacistSignatures = {
  ...
  document.getElementById("generateCertificate").addEventListener("click", function() {
    ...
    // After generating certificates, enable the download button
    document.getElementById("downloadAll").disabled = false;
    console.log("Certificates generated. Download button enabled.");
  });
  ...
  document.getElementById("downloadAll").addEventListener("click", function() {
    ...
    // Create an empty array to store the HTML content of each certificate
    const htmlContent = [];
  
    certificates.forEach((certificate, index) => {
      htmlContent.push(certificate.innerHTML);
    });
  
    // Join all the HTML content together
    const combinedHtml = htmlContent.join('');
  
    // Convert the combined HTML content to PDF and save it
    html2pdf().from(combinedHtml).save().then(() => {
